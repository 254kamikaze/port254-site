// app.js - MVP frontend wiring for port254 template (fake-data mode)
const state = {
  all: [],
  shown: [],
  q: '',
...
async function init(){
  try {
    ...
    if (!res.ok) throw new Error('no tests.json found, loading fallback');
    state.all = await res.json();
  } catch(e){
    console.warn('Failed loading tests.json; using embedded fake data', e);
    state.all = embeddedFakeData();
  }
  bindUI();
  applyAndRender();
}
...
function renderCards(list){
  ...
  els.grid.innerHTML = list.map(t=>cardHTML(t)).join('');
}
...
function embeddedFakeData(){
  // A small set of fake records for UI development (no real telemetry)
  return [
    {
      "id":"ot-001",
      "title":"S7 Session Failure Storm (simulated)",
      "domain":"ot",
      "category":"Network",
      "criticality":"high",
      "added_at":"2025-10-07T00:00:00Z",
      "tags":["s7","plc","conpot"],
      "mitre_attack_ids":["T1110"],
      "iec62443_controls":["SR 1.1"],
      "description":"Simulated repeated S7 session failures from a single IP to test UI behavior.",
      "kibana_url":"#",
      "youtube_url":"#",
    ...
    {
      "id":"ot-002",
      "title":"Modbus Read Flood (simulated)",
      "domain":"ot",
      ...
      "iec62443_controls":["SR 3.2"],
      "description":"Fake event stream that mimics high-frequency Modbus reads to a virtual PLC.",
      "kibana_url":"#",
      "youtube_url":"#",
      "kml_url":"kml/plant.kml"
    },
    {
      "id":"fin-001",
      "title":"Cardholder Geo-Velocity Spike (demo)",
      "domain":"fintech",
      "category":"Fraud",
      "criticality":"critical",
      "added_at":"2025-10-06T00:00:00Z",
      "tags":["card","velocity","geo"],
      "mitre_attack_ids":["T1078"],
      "iec62443_controls":["SR 5.2"],
      "description":"Demo-only record showing distant card usage within a short timeframe.",
      "kibana_url":"#",
      "youtube_url":"#",
    ...
    },
    {
      ...
      "description":"Simulated ATO login pattern for UI layout testing.",
      "kibana_url":"#",
      "youtube_url":"#",
      "kml_url":"kml/branches.kml"
    }
