...
const ES_HOST = 'http://192.168.1.12:9200';
const ES_USER = 'elastic';
const ES_PASS = 'fonJULVM40vX6HccsDub';
const ES_INDEX = 'honeypot-events-*';
...
async function esQuery(query) {
    ...
    if (!response.ok) {
        console.error('ES query failed:', response.status);
        throw new Error(`ES query failed: ${response.status}`);
    }
